...
class TestWaitAll:
    def test_wait_all_blocks_until_complete_thread(self):
        tq = ThreadTaskQueue(name="WaitQueue", max_workers=2)

        tq.submit(slow_task, duration=0.1)
        tq.submit(slow_task, duration=0.1)

        start = time.perf_counter()
        tq.wait_all(timeout=5.0)
    ...
    def test_wait_all_blocks_until_complete_process(self):
        tq = ProcessTaskQueue(name="WaitProcessQueue", max_workers=2)
    ...
    def test_wait_all_empty_queue(self):
        tq = ThreadTaskQueue(name="EmptyQueue", max_workers=1)

        tq.wait_all(timeout=1.0)

        tq.close()


class TestStreamCompleted:
    def test_stream_completed_yields_results_in_order_thread(self):
        tq = ThreadTaskQueue(name="StreamQueue", max_workers=2, retain_results=True)

        n = 5
        for i in range(n):
            tq.submit(fast_task, x=i)

        results = {}
        for task_id, result in tq.stream_completed(timeout=5.0):
            results[task_id] = result

        assert len(results) == n
        tq.close()

    def test_stream_completed_yields_errors_thread(self):
        tq = ThreadTaskQueue(
            name="StreamErrorQueue", max_workers=2, retain_results=True
        )

        tq.submit(failing_task, msg="error1")
    ...
    def test_stream_completed_without_retain_warns(self):
        ...
        with pytest.MonkeyPatch.context() as m:
            logged = []
            m.setattr(
                logging.Logger, "warning", lambda self, msg, *args: logged.append(msg)
            )
    ...
    def test_stream_completed_timeout_stops(self):
        tq = ThreadTaskQueue(
            name="StreamTimeoutQueue", max_workers=1, retain_results=True
        )

        tq.submit(slow_task, duration=0.1)

        results = list(tq.stream_completed(timeout=5.0))
...
class TestGetStatus:
    def test_get_status_running_then_completed(self):
        tq = ThreadTaskQueue(name="StatusQueue", max_workers=1, retain_results=True)

        task_id = tq.submit(slow_task, duration=0.3)

        status_running = tq.get_status(task_id)
        assert status_running == TaskStatus.RUNNING

        tq.wait_all(timeout=5.0)

        status_done = tq.get_status(task_id)
    ...
    def test_get_status_failed(self):
        tq = ThreadTaskQueue(
            name="FailedStatusQueue", max_workers=1, retain_results=True
        )

        task_id = tq.submit(failing_task, msg="fail")
    ...
    def test_get_status_unknown_task_raises(self):
        tq = ThreadTaskQueue(name="UnknownQueue", max_workers=1)

        with pytest.raises(ValueError, match="Unknown task ID"):
            tq.get_status("non-existent-id")
...
class TestCancel:
    ...
    def test_cancel_nonexistent_task_returns_false(self):
        tq = ThreadTaskQueue(name="NoCancelQueue", max_workers=1)

        result = tq.cancel("nonexistent")
...
class TestGetAllStatuses:
    def test_get_all_statuses_multiple_tasks(self):
        tq = ThreadTaskQueue(name="AllStatusQueue", max_workers=2, retain_results=True)

        t1 = tq.submit(fast_task, x=1)
        t2 = tq.submit(slow_task, duration=0.2)
        t3 = tq.submit(failing_task, msg="err")

        tq.wait_all(timeout=2.0)

        statuses = tq.get_all_statuses()
